...
export const useResumeStore = defineStore('resume', () => {
  ...
  async function changeExperience({
    ...
    try {
      ...
      const { data } = await API.changeExperience(
        { position, company, responsibility, start, finish, webSite },
        _id
      );
  ...
  async function changeCertificate({ position, company, start, finish, webSite, _id }) {
    try {
      setLoadingCertificate(true);

      const { data } = await API.changeCertificate(
        { position, company, start, finish, webSite },
        _id
      );
